...
return ( 
    <div className="mn-c-learning-material-section">
	    <LearningMaterial 
	    	...
	    	title={title} 
	    	pages={pages} 
	    	handleDelete={() => {materialDelete(index)}}
	    /> 
	    { sections != null && sections.length != 0 &&
		    <div className="section-list">
			    { sections != null && sections.map((section, i) => {
	    ...
	    { type == "textbook" && <MaterialSectionAdd 
		    			onClick={() => {setRoot(index); showSection(true);}}
		    /> 
	    }
    </div>
)}
...
const MaterialWrapper = ({
  ...
  return (
	...
	{materials.map((material, i) => {
	    const sections = (material.type == "textbook" ? material.sections : null)
	    return <LearningMaterialSection 
		key={i} 
		index={i} 
		type={material.type} 
